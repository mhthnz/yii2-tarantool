...
trait MigrateControllerTestTrait
{
    ...
    {
        $this->migrationNamespace = 'mhthnz\tarantool\tests\migrations';
        $this->migrationPath = TESTS_PATH . '/migrations';
    ...
    public function tearDownMigrationPath()
    {
        ...
        FileHelper::removeDirectory(Yii::getAlias('@mhthnz/tarantool/tests/app_migrations'));
        FileHelper::removeDirectory(Yii::getAlias('@mhthnz/tarantool/tests/ext_migrations'));
    }
...
CODE;
    ...
    protected function createMigrationWithContent($name, string $up = "", string $down = "", $date = null)
    {
        if ($date === null) {
            $date = gmdate('ymd_His');
        }
...
CODE;
    ...
    public function testCreate()
    {
        ...
        $this->assertTrue(strpos(basename($files[0]), $migrationName) !== false);
    }
    ...
    public function testHistory()
    {
        ...
        $this->assertTrue(stripos($output, 'No migration') !== false);
        ...
        $output = $this->runMigrateControllerAction('history');
        $this->assertSame(ExitCode::OK, $this->getExitCode());
        $this->assertTrue(stripos($output, '_test_history1') !== false);
        $this->assertTrue(stripos($output, '_test_history2') !== false);
    }
    ...
    public function testNew()
    {
        ...
        $this->assertSame(ExitCode::OK, $this->getExitCode());
        $this->assertTrue(stripos($output, '_test_new1') !== false);
        ...
        $output = $this->runMigrateControllerAction('new');
        $this->assertSame(ExitCode::OK, $this->getExitCode());
        $this->assertFalse(stripos($output, '_test_new1') !== false);
    }
    ...
    public function testNamespaceCreate()
    {
        ...
        $this->assertTrue(stripos($fileContent, "namespace {$this->migrationNamespace};") !== false);
        $this->checkRegex('/class M[0-9]{12}' . ucfirst($migrationName) . '/s', $fileContent);
        unlink($files[0]);
        ...
        $files = FileHelper::findFiles($this->migrationPath);
        $fileContent = file_get_contents($files[0]);
        $this->assertFalse(stripos($fileContent, "namespace {$this->migrationNamespace};") !== false);
    }
    ...
    {
        $a = $this->createNamespaceMigration('nsTest1');
        $b = $this->createNamespaceMigration('nsTest2');
    ...
    public function testNamespaceHistory()
    {
        ...
        $output = $this->runMigrateControllerAction('history', [], $controllerConfig);
        $this->assertSame(ExitCode::OK, $this->getExitCode());
        $this->checkRegex('/' . preg_quote($this->migrationNamespace) . '.*History1/s', $output);
        $this->checkRegex('/' . preg_quote($this->migrationNamespace) . '.*History2/s', $output);
    }
    ...
    {
        FileHelper::createDirectory(Yii::getAlias('@mhthnz/tarantool/tests/app_migrations'));
        FileHelper::createDirectory(Yii::getAlias('@mhthnz/tarantool/tests/ext_migrations'));

        $controllerConfig = [
            'migrationPath' => [$appPath = '@mhthnz/tarantool/tests/app_migrations', $extensionPath = '@mhthnz/tarantool/tests/ext_migrations'],
            'migrationNamespaces' => [$this->migrationNamespace],
        ];
