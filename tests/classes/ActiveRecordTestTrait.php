...
trait ActiveRecordTestTrait
{
    ...
    public function testFindLimit()
    {
        ...
        // Without limit
        $customers = $customerClass::find()->orderBy('id')->offset(1)->all();
        ...
        // Without limit
        $customers = $customerClass::find()->orderBy('id')->offset(2)->all();
        $this->assertCount(1, $customers);
        $this->assertEquals('user3', $customers[0]->name);
        ...
        // Without limit
        $customers = $customerClass::find()->offset(3)->all();
        $this->assertCount(0, $customers);
        ...
        $customer = $customerClass::find()->orderBy('id')->limit(1)->offset(0)->one();
        $this->assertEquals('user1', $customer->name);

        $customer = $customerClass::find()->orderBy('id')->limit(1)->offset(1)->one();
        $this->assertEquals('user2', $customer->name);

        $customer = $customerClass::find()->orderBy('id')->limit(1)->offset(2)->one();
        $this->assertEquals('user3', $customer->name);

        $customer = $customerClass::find()->offset(3)->limit(1)->one();
        $this->assertNull($customer);
    }
    ...
    public function testExists()
    {
        ...
        $this->assertTrue($customerClass::find()->where(['id' => [2, 3]])->limit(1000)->offset(1)->exists());
        $this->assertFalse($customerClass::find()->where(['id' => [2, 3]])->limit(1000)->offset(2)->exists());
    }
    ...
    public function testBooleanAttribute()
    {
        ...
        $customer->refresh();
        $this->assertEquals(true, $customer->bool_status);

        $customer->bool_status = false;
        $customer->save(false);

        $customer->refresh();
        $this->assertEquals(false, $customer->bool_status);
